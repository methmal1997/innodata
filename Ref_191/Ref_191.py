...
try:
    ...
    for i, url_url_id in enumerate(url_list):
        ...
        try:
            ...
            nav = current_soup.find('ul', class_='nav')
            current_issue_link = nav.find('a', href=lambda href: href and 'current' in href)['href']

            response = requests.get(current_issue_link, timeout=50, allow_redirects=True)
            current_soup = BeautifulSoup(response.content, 'html.parser')
            script_tag = current_soup.find('script', language="javascript")
            ...
            if script_content:
                ...
                url_parts = redirect_url.split('/')[-1]

            current_issue_link = "http://www.zpwz.net/zgptwkzzen/article/issue/"+url_parts
            response = requests.get(current_issue_link, timeout=50, allow_redirects=True)
            current_soup = BeautifulSoup(response.content, 'html.parser')
            year, volume, issue = url_parts.split('_')
            print(year)
            print(volume)
            print(issue)
            pdf_list = []
            Articles = current_soup.findAll("li",class_="article_line")
            for artticle in Articles:
                Article_title = artticle.find("div",class_="article_title").text
                article_position = artticle.find('p', class_='article_position')

                text = article_position.text
                page_range_match = re.search(r'\d{3,}-\d{3,}', text)
                page_range = page_range_match.group(0) if page_range_match else None

                doi_tag = article_position.find('a', href=True)
                DOI = doi_tag.text if doi_tag else None
                pdf_link_tag = artticle.find('a', class_='btn_pdf')
                Pdf_link = "http://www.zpwz.net/"+ pdf_link_tag['href']
