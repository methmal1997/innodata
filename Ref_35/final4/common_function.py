...
def send_email(subject, body, attachments,Sending_address,to_email_list,cc_email_list,port):
    ...
    def output_TOC_name(current_path):
        return os.path.join(current_path, os.path.basename(current_path) + '.html'), str(
            os.path.basename(current_path) + '.html')
    ...
    def read_ini_file(ini_path):
        Download_path, Download_user, Source_id, Email_sent, Check_duplicate, Sending_address, Receiving_address, CC, Port = get_ini_file_values(
            ini_path)
        if not os.path.exists(Download_path):
            os.makedirs(Download_path)
        return Download_path, Email_sent, Check_duplicate, Download_user

    def attachment_for_email(url_id, duplicate_list, error_list, completed_list, pdf_count, ini_path, attachment,
                             date_for_email, time_for_email, Ref_value):
        Download_path, Download_user, Source_id, Email_sent, Check_duplicate, Sending_address, Receiving_address, CC, Port = get_ini_file_values(
            ini_path)
        compose_email_to_send(url_id, duplicate_list, error_list, completed_list, pdf_count, attachment, date_for_email,
                              time_for_email, Sending_address, Receiving_address, CC, Port, Ref_value)
    ...
    def compose_email_to_send(url_id, duplicate_list, error_list, completed_list, pdf_count, attachment, date_for_email,
                              time_for_email, Sending_address, to_email_list, cc_email_list, port, Ref_value):
        subject, body = email_body(str(date_for_email), str(time_for_email), duplicate_list, error_list, completed_list,
                                   pdf_count, url_id, Ref_value)
    ...
    def email_body_html(email_date, email_time, skipped, errors, completed_list, download_count, source_id, Ref_value,
                        attachment, current_out):
        out_html_file = output_email_file(current_out)

        subject = '<h3>{}</h3>'.format(
            '{} downloaded details ({}) Ref_{}'.format(source_id, email_date + ' ' + email_time, Ref_value))
        body = ""
    ...
    def save_email_body(url_id, duplicate_list, error_list, completed_list, pdf_count, attachment, date_for_email,
                        time_for_email, Sending_address, to_email_list, cc_email_list, port, Ref_value,
                        selected_directory):
        subject, body = email_body(str(date_for_email), str(time_for_email), duplicate_list, error_list, completed_list,
                                   pdf_count, url_id, Ref_value)
