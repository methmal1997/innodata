    ...
    for i,url_url_id in enumerate(url_list):
        try:
            try:
                ...
                try:
                        ...
                        while proxy:
                            ...
                            if proxy_number < 10:
                                try:
                                    ...
                                    response_2 = requests.get('https://api.ipify.org?format=json',proxies=formatted_proxies[proxy_number], headers=headers,timeout=20)
                    ...
                    try:
                        try:
                            ...
                            response = requests.get(current_issue_link, proxies=formatted_proxies[proxy_number], headers=headers,timeout=20)
                            current_soup_2 = BeautifulSoup(response.content, 'html.parser')
                            volume_issue = current_soup_2.find('span', class_='volume issue').text.split(', ')
                    except:
                        try:
                ...
                Error_message = "Error in the urls text file  or nother error:" + str(error)
                print(Error_message)
                error_list.append(Error_message)
        except:
            pass
